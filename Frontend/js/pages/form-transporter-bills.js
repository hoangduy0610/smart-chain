$(document).ready(function () {
    ...
    const prefetch_id = params.get('trb');
    let batchId = params.get('id')

    if (!!prefetch_id) {
        $(".loader-container").addClass('active');
        $.ajax({
            url: fillEndpointPlaceholder(API_ENDPOINT.TRANSPORTER_BILLS.GET_TRANSPORTER_BILLS, { id: prefetch_id }),
            method: 'GET',
            headers: {
    ...
    $("#submit-transporter-form").click(function () {
        ...
        $.ajax({
            url: !!prefetch_id ? fillEndpointPlaceholder(API_ENDPOINT.TRANSPORTER_BILLS.UPDATE_TRANSPORTER_BILLS, { id: prefetch_id }) : API_ENDPOINT.TRANSPORTER_BILLS.CREATE_TRANSPORTER_BILLS,
            method: !!prefetch_id ? 'PUT' : 'POST',
            headers: {
                'Authorization': ACCESS_TOKEN,
            },
            data: !!prefetch_id ? {
            ...
            success: function (response) {
                const actionText = !!prefetch_id ?
                    `Nhà vận chuyển ${userInfo.name} đã cập nhật lộ trình cho đơn vận chuyển: từ ${departure} đến ${destination}.` :
                    `Nhà vận chuyển ${userInfo.name} đã tạo đơn vận chuyển từ ${departure} đến ${destination}.`;
                $.ajax({
